...
> **Note (2026-04-29):** The server itself + Workflow SDK adoption + minimum-viable routes (`POST /runs`, `GET /runs/:id/stream`, `POST /signals/:id/resolve`, `GET /health`) landed in **P1** as part of the Nitro adoption decision. `GET /api/v1/runs` (run list reading IndexStore) followed in P1 commit 11. Phase 3 now covers the *rest* of the HTTP API surface, the polished CLI subcommand client, and OpenAPI generation. Hono is replaced by Nitro/h3 (Nitro's underlying server). `pnpm orchestrator serve` is a thin wrapper around `nitro dev`.
>
> **OpenAPI scaffolding deferred (2026-04-30, task #11):** Tried to enable Nitro's `experimental.openAPI` flag (would serve `/_openapi.json` + `/_swagger` + `/_scalar` automatically). Two blockers in `nitro@3.0.1-alpha.1`: (1) the route-meta extractor crashes on `workflow/nitro`'s virtual handlers — `Could not load \x00nitro-handler-meta:\x00virtual:#workflow/webhook.mjs ... null bytes` because virtual handler ids contain null bytes that h3's path resolver rejects; (2) every route file with a `defineRouteMeta(...)` call hit a TDZ `Cannot access '<route>$1' before initialization` regardless of where the call sat in the file (before or after the export). Both look like nitro-3-alpha codegen bugs. Revisit when nitro ships a stable 3.x or moves to 4. The annotation TSDoc on each route's source still works as a human reference; we're only missing the auto-generated spec endpoint.
