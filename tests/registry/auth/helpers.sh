...
# Registry has its own Tailscale sidecar identity
REGISTRY_FQDN=$(ts_fqdn "otacon-registry")
REGISTRY_URL="${OTACON_REGISTRY_URL:-http://${REGISTRY_FQDN}:9080}"

# Admin binds to host port 9090, reached via Pi's existing host-level Tailscale
ADMIN_URL="${OTACON_ADMIN_URL:-http://${PI_FQDN}:9090}"
ADMIN_TOKEN="${OTACON_ADMIN_TOKEN:-}"

export REGISTRY_URL ADMIN_URL ADMIN_TOKEN
...
test_host_id() {
    echo "fake-test-node-$(uuidgen 2>/dev/null || python3 -c 'import uuid; print(uuid.uuid4())')"
}

# Register a test node, set PENDING_ID
register_test_node() {
    local host_id="$1"
    local result
    ...
    if [ "$status" != "200" ] && [ "$status" != "201" ]; then
        echo "Registration failed: status=$status body=$body" >&2
        return 1
    fi
...
# Approve a pending registration, returns the token
approve_registration() {
    local pending_id="$1"
    local result
    result=$(http_post "$ADMIN_URL/api/v1/auth/registrations/$pending_id/approve" \
        '{}' "$ADMIN_TOKEN")
    local status
    status=$(get_status "$result")
    local body
    ...
    if [ "$status" != "200" ] && [ "$status" != "201" ]; then
        echo "Approval failed: status=$status body=$body" >&2
        return 1
    fi
    echo "$body" | jq -r '.token // empty'
}

# Find the token_id for a node token by querying admin list_tokens.
# The poll response only returns the raw token, not the token_id.
# We need the token_id for revocation. Match by token_prefix.
find_token_id() {
    local raw_token="$1"
...
# Full flow: register -> poll (bg) -> approve -> get token + token_id
# Sets: NODE_TOKEN, TOKEN_ID
...
get_node_token() {
    ...
    (
        ...
        st=$(curl -s -o "$poll_tmpfile" -w '%{http_code}' \
            -X POST --max-time 30 \
            "$REGISTRY_URL/api/v1/auth/poll/$PENDING_ID" 2>/dev/null) || st="000"
        echo "$st" > "$poll_status_file"
    ) &
    local poll_pid=$!
    sleep 2

    http_post "$ADMIN_URL/api/v1/auth/registrations/$PENDING_ID/approve" '{}' "$ADMIN_TOKEN" >/dev/null
...
revoke_token() {
    local tid="$1"
    http_post "$ADMIN_URL/api/v1/auth/tokens/$tid/revoke" '{}' "$ADMIN_TOKEN" >/dev/null 2>&1
}
