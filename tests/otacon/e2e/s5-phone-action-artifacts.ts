 ...
 *     files on disk. Per implementer: paths are relative to OTACON_DATA_DIR
 *     when set absolute, else relative to process.cwd(). The harness sets
 *     OTACON_DATA_DIR to an absolute path so paths in events.jsonl are
 *     absolute and stable.
 *
 * Run:
...
// Reuse the orchestrator's png-diff helper for perceptual hash comparison
// (same approach phase2-xhs-actions.ts uses — keeps the threshold consistent).
import { pHash, hammingDistance } from '../../orchestrator/e2e/helpers/png-diff.js'
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const a of phoneActions) {
      ...
      // Resolve paths: implementer says set OTACON_DATA_DIR absolute and
