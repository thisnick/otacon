/**
 * Shared helpers for orchestrator e2e tests.
 *
 * All `runOrchestrator` invocations target the orchestrator CLI via
 * `pnpm --filter orchestrator orchestrator ...`.
 *
...
export function makeFixture(name: string): SpikeFixture {
  const tmpRoot =
    ...
    fs.mkdtempSync(path.join(os.tmpdir(), `orchestrator-${name}-`))
...
// ---------------------------------------------------------------------------
// Bootstrap — calls the orchestrator's seed script against fix.dataDir.
// `pnpm --filter orchestrator seed` with ORCHESTRATOR_DATA_DIR set produces
// workspace + team + S2 marker file.
// ---------------------------------------------------------------------------

export function seedSpike(fix: SpikeFixture): { status: number; stdout: string; stderr: string } {
  const res = spawnSync('pnpm', ['--filter', 'orchestrator', 'seed'], {
    cwd: REPO_ROOT,
...
export function runOtaconRun(
  ...
  const argv: string[] = [
    '--filter', 'orchestrator', 'orchestrator', 'run',
  ...
  const res = spawnSync('pnpm', argv, {
    ...
    env: {
      ...process.env,
      ORCHESTRATOR_DATA_DIR: fix.dataDir,
      ...extraEnv,
    },
    encoding: 'utf-8',
    stdio: ['ignore', 'pipe', 'pipe'],
