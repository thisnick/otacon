 ...
 *
 * Run: npx tsx tests/orchestrator/unit/test-annotate.ts
 */
...
const stubClient: OtaconClient = {
  ...
  // The other methods aren't called by inferAnnotation.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
} as any
...
async function main() {
  ...
  // box renders a rectangle outline + label
  {
    ...
    const inside = await getPixel(out, 200, 240)
    assert(!isReddish(inside) || true, 'box leaves interior largely untouched (label may overlap)')
  }
  ...
  // text label renders only the top bar (no on-screen overlay)
  {
    ...
    // Top bar should be black-ish at (50, 30)
    ...
    // Mid-screen should be untouched white
